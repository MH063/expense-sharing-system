/**
 * 测试数据库初始化（PostgreSQL + 原生 SQL 脚本）
 * - 使用 config/db.js 的 pg 连接池
...
async function initTestDatabase() {
  ...
  try {
    ...
    console.error('测试数据库连接失败，请检查 .env.test 配置:', e.message);
    throw e;
  }
...
/**
 * 清理测试数据库：TRUNCATE public schema 下所有用户表（保留扩展/函数）
 */
...
async function closeTestDatabase() {
  try {
    ...
    console.error('关闭测试数据库连接池失败:', e.message);
    // 不再抛出，避免影响后续测试套件
  }
}
