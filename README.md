# 寝室费用分摊记账系统

一个基于Node.js + Express + PostgreSQL + Vue 3的现代化寝室费用分摊记账系统，帮助室友间公平分摊日常开销。

## 🚀 功能特性

### 用户功能
- ✅ 用户注册与登录（JWT认证）
- ✅ 寝室创建与管理（邀请码机制）
- ✅ 费用记录与分类管理（多种费用类型）
- ✅ 多种费用分摊方式（平均、按天数、自定义、按面积）
- ✅ 账单生成与支付确认（支持微信/支付宝收款码）
- ✅ 实时通知提醒（WebSocket）
- ✅ 费用统计与趋势分析（ECharts图表）
- ✅ 多设备同步（响应式设计）
- ✅ 争议处理与审核流程

### 管理功能
- ✅ 系统管理：用户管理、寝室管理、系统配置
- ✅ 数据统计：费用统计、用户活跃度、系统使用情况
- ✅ 审核管理：费用审核、争议处理、异常监控
- ✅ 日志管理：操作日志、系统日志、审计追踪
- ✅ 文档管理：API文档、数据库文档、需求文档

## 🛠️ 技术栈

### 后端技术
- **运行环境**: Node.js 16.x+
- **Web框架**: Express.js 4.18.2
- **数据库**: PostgreSQL 14+
- **缓存**: Redis 6+ (可选)
- **认证**: JWT (JSON Web Tokens)
- **实时通信**: WebSocket (ws 8.13.0)
- **文件上传**: Multer 2.0.2
- **日志管理**: Winston 3.10.0
- **安全**: Helmet 7.0.0, bcryptjs 2.4.3
- **测试框架**: Jest 29.7.0, Supertest 6.3.3

### 前端技术
- **框架**: Vue 3.3.4 + Composition API
- **构建工具**: Vite 4.4.5
- **UI组件库**: Element Plus 2.3.8
- **状态管理**: Pinia 2.1.6 + Vuex 4.0.2
- **路由管理**: Vue Router 4.2.4
- **HTTP客户端**: Axios 1.4.0
- **图表库**: ECharts 6.0.0
- **测试框架**: Vitest 0.34.6

### 管理后台技术
- **框架**: Vue 3.3.4
- **UI组件库**: Element Plus 2.3.8
- **图表库**: ECharts 6.0.0
- **构建工具**: Vite 4.4.5

## 📦 快速开始

### 环境要求
- Node.js 16.x 或更高版本
- PostgreSQL 14+ 数据库
- Redis 6+ (可选，用于缓存和会话存储)

### 本地开发

1. 克隆项目
```bash
git clone https://github.com/MH063/expense-sharing-system.git
cd expense-sharing-system
```

2. 安装后端依赖
```bash
cd Server-side
npm install
```

3. 配置后端环境变量
```bash
# 复制环境变量模板
cp .env.development.example .env.development

# 编辑 .env.development 文件，配置数据库连接和其他环境变量
# 确保以下配置正确：
# DB_HOST=localhost
# DB_PORT=5432
# DB_USER=postgres
# DB_PASSWORD=your_password
# DB_NAME=test_expense_system
```

4. 初始化数据库
```bash
# 创建测试数据库（在PostgreSQL中执行）
psql -U postgres -c "CREATE DATABASE test_expense_system;"

# 初始化数据库表结构
npm run init:testdb
```

5. 启动后端服务
```bash
# 开发模式启动
npm run dev

# 或者生产模式启动
npm run prod
```

6. 安装前端依赖
```bash
cd ../Client\ application
npm install
```

7. 启动前端应用
```bash
npm run dev
```

8. 安装管理后台依赖（可选）
```bash
cd ../Admin\ panel
npm install
```

9. 启动管理后台
```bash
npm run dev
```

### 访问地址

- 前端应用: http://localhost:8000
- 后端API: http://localhost:4000
- 管理后台: http://localhost:8100
- API健康检查: http://localhost:4000/health

### 生产部署

1. 构建前端应用
```bash
cd Client\ application
npm run build
```

2. 构建管理后台
```bash
cd ../Admin\ panel
npm run build
```

3. 启动后端服务
```bash
cd ../Server-side
npm run prod
```

## 🔧 开发指南

### 项目结构
```
expense-sharing-system/
├── Client application/          # 前端应用 (Vue 3)
│   ├── public/                  # 静态资源
│   ├── src/
│   │   ├── api/                 # API请求模块
│   │   ├── assets/              # 资源文件
│   │   ├── components/          # 公共组件
│   │   ├── router/              # 路由配置
│   │   ├── stores/              # Pinia状态管理
│   │   ├── utils/               # 工具函数
│   │   ├── views/               # 页面组件
│   │   │   ├── analytics/       # 数据分析页面
│   │   │   ├── auth/            # 认证相关页面
│   │   │   ├── bills/           # 账单管理页面
│   │   │   ├── disputes/        # 争议处理页面
│   │   │   ├── expenses/        # 支出管理页面
│   │   │   ├── reviews/         # 评论管理页面
│   │   │   └── rooms/           # 房间管理页面
│   │   ├── App.vue              # 根组件
│   │   └── main.js              # 入口文件
│   ├── package.json
│   └── vite.config.js
├── Admin panel/                 # 管理后台 (Vue 3)
│   ├── public/                  # 静态资源
│   ├── src/
│   │   ├── api/                 # API请求模块
│   │   ├── assets/              # 资源文件
│   │   ├── components/          # 公共组件
│   │   ├── router/              # 路由配置
│   │   ├── stores/              # Pinia状态管理
│   │   ├── utils/               # 工具函数
│   │   ├── views/               # 页面组件
│   │   │   ├── config/          # 系统配置页面
│   │   │   ├── docs/            # 文档管理页面
│   │   │   ├── review/          # 审核管理页面
│   │   │   └── system/          # 系统管理页面
│   │   ├── App.vue              # 根组件
│   │   └── main.js              # 入口文件
│   ├── package.json
│   └── vite.config.js
├── Server-side/                 # 后端服务 (Node.js)
│   ├── src/
│   │   ├── controllers/         # 控制器
│   │   │   ├── bill-controller.js
│   │   │   ├── expense-controller.js
│   │   │   ├── expense-type-controller.js
│   │   │   ├── room-controller.js
│   │   │   ├── stats-controller.js
│   │   │   └── user-controller.js
│   │   ├── middleware/          # 中间件
│   │   ├── models/              # 数据模型
│   │   ├── routes/              # 路由定义
│   │   ├── services/            # 业务逻辑
│   │   ├── utils/               # 工具函数
│   │   └── app.js               # 应用入口
│   ├── tests/                   # 测试文件
│   │   ├── integration/         # 集成测试
│   │   └── unit/                # 单元测试
│   ├── scripts/                 # 脚本文件
│   │   └── init-test-database.sql
│   ├── package.json
│   └── .env.development.example
├── docs/                        # 项目文档
└── README.md
```

### API 文档

#### 认证相关
- `POST /api/auth/register` - 用户注册
- `POST /api/auth/login` - 用户登录
- `POST /api/auth/logout` - 用户登出
- `GET /api/auth/profile` - 获取用户信息
- `PUT /api/auth/profile` - 更新用户信息

#### 房间管理
- `GET /api/rooms` - 获取用户房间列表
- `POST /api/rooms` - 创建房间
- `GET /api/rooms/:id` - 获取房间详情
- `PUT /api/rooms/:id` - 更新房间信息
- `DELETE /api/rooms/:id` - 删除房间
- `POST /api/rooms/:id/join` - 加入房间

#### 费用管理
- `GET /api/expenses` - 获取费用列表
- `POST /api/expenses` - 创建费用
- `GET /api/expenses/:id` - 获取费用详情
- `PUT /api/expenses/:id` - 更新费用
- `DELETE /api/expenses/:id` - 删除费用

#### 账单管理
- `GET /api/bills` - 获取账单列表
- `POST /api/bills` - 创建账单
- `GET /api/bills/:id` - 获取账单详情
- `PUT /api/bills/:id` - 更新账单
- `POST /api/bills/:id/confirm` - 确认支付

#### 费用类型
- `GET /api/expense-types` - 获取费用类型列表
- `POST /api/expense-types` - 创建费用类型
- `PUT /api/expense-types/:id` - 更新费用类型
- `DELETE /api/expense-types/:id` - 删除费用类型

#### 统计数据
- `GET /api/stats/overview` - 获取总览统计
- `GET /api/stats/expenses` - 获取费用统计
- `GET /api/stats/users` - 获取用户统计

#### 收款码管理（已实现）
- `POST /api/qr-codes/upload` - 上传收款码（表单字段：qr_image；Body 字段：qr_type=wechat|alipay）
- `GET /api/qr-codes` - 获取当前用户的收款码列表
- `PATCH /api/qr-codes/:id/status` - 激活/停用收款码（Body：{ is_active: true|false }）
- `PATCH /api/qr-codes/:id/default` - 设置默认收款码
- `GET /api/qr-codes/default?qr_type=wechat|alipay` - 获取默认收款码
- `DELETE /api/qr-codes/:id` - 删除收款码

#### 支付流程（已实现，含离线支付能力）
- `GET /api/payments/bills/:billId/qr-code?qr_type=wechat|alipay` - 获取账单收款码与支付信息
- `POST /api/payments/bills/:billId/confirm` - 付款人确认支付（Body：{ payment_method, transaction_id, payment_time }）
- `GET /api/payments/bills/:billId/status` - 查询账单支付状态
- `POST /api/payments/offline` - 创建离线支付记录
- `POST /api/payments/:paymentId/sync` - 同步离线支付凭证
- `GET /api/payments/offline` - 获取用户离线支付记录
- `GET /api/payments/offline/pending-sync` - 获取待同步的离线支付
- `PATCH /api/payments/:paymentId/sync-failed` - 标记离线支付同步失败
- `POST /api/payments/:paymentId/retry-sync` - 重试同步失败的离线支付

### 数据库设计

#### 主要表结构
- `users` - 用户表
- `rooms` - 房间表
- `room_members` - 房间成员表
- `expense_types` - 费用类型表
- `expenses` - 费用记录表
- `expense_shares` - 费用分摊表
- `bills` - 账单表
- `bill_items` - 账单项目表
- `payments` - 支付记录表
- `payment_methods` - 支付方式表
- `disputes` - 争议表
- `notifications` - 通知表

### 测试

#### 运行测试
```bash
# 运行所有测试
npm test

# 运行单元测试
npm run test:unit

# 运行集成测试
npm run test:integration

# 生成测试覆盖率报告
npm run test:coverage
```

#### 测试覆盖范围
- 用户认证API测试
- 房间管理API测试
- 费用管理API测试
- 账单管理API测试
- 统计数据API测试
- 费用类型API测试

### 代码规范

#### 后端代码规范
- 使用ESLint进行代码检查
- 遵循Airbnb JavaScript风格指南
- 使用async/await处理异步操作
- 统一的错误处理机制
- 详细的API文档注释

#### 前端代码规范
- 使用ESLint + Prettier进行代码格式化
- 组件命名使用PascalCase
- 文件命名使用kebab-case
- 使用Vue 3 Composition API
- 统一的API请求封装
- 提交信息使用约定式提交

## 📄 文档

- [API文档](./Server-side/docs/API.md)
- [部署指南](./Server-side/docs/DEPLOYMENT.md)
- [技术问题记录](./Server-side/docs/TECHNICAL_ISSUES.md)

## 🧪 测试

运行测试：
```bash
cd Server-side
npm test
```

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📝 许可证

MIT License

## 🙏 致谢

感谢以下开源项目：
- Express.js
- React
- Vite
- PostgreSQL
- Vue.js
- Element Plus