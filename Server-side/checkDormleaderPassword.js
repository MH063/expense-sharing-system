...
async function checkUserPassword() {
  try {
    // 查询用户dormleader001的密码哈希
    const userResult = await pool.query('SELECT id, username, password_hash FROM users WHERE username = $1', ['dormleader001']);
    ...
    if (userResult.rows.length > 0) {
      ...
      console.log('未找到用户dormleader001');
    }
  } catch (error) {
