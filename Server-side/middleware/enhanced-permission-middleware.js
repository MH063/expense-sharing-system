...
const checkPermission = (permission) => {
  return async (req, res, next) => {
    try {
      ...
      // 使用权限服务检查权限
      const hasPermission = await permissionService.checkUserPermission(userId, permission);
...
const checkAnyPermission = (permissions) => {
  return async (req, res, next) => {
    try {
      ...
      // 使用权限服务检查权限
      const hasPermission = await permissionService.checkUserAnyPermission(userId, permissions);
...
const checkAllPermissions = (permissions) => {
  return async (req, res, next) => {
    try {
      ...
      // 使用权限服务检查权限
      const hasAllPermissions = await permissionService.checkUserAllPermissions(userId, permissions);
      
      if (!hasAllPermissions) {
        logger.warn(`用户 ${userId} 尝试访问需要权限 ${permissions.join(' 和 ')} 的资源但权限不足`);
        return res.status(403).json({
          success: false,
          message: '权限不足'
        });
  ...
  return async (req, res, next) => {
    try {
      ...
      for (const role of requiredRoles) {
        hasRole = await permissionService.checkUserRole(userId, role);
        if (hasRole) break;
      }
    ...
    try {
      ...
      try {
        ...
        if (ownerResult.rows.length > 0) {
          isOwner = ownerResult.rows[0].user_id.toString() === userId.toString();
        }
      } catch (error) {
